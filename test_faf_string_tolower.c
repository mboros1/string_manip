...
static void test_empty_string(void) {
    ...
    faf_string* result = faf_string_tolower(test_pool, empty);
...
static void test_single_char_uppercase(void) {
    ...
    faf_string* result = faf_string_tolower(test_pool, str);
    
    ASSERT_TRUE(result != NULL, "Result should not be NULL");
    ASSERT_INT_EQ(1, result->end - result->start, "Length should be 1");
    ASSERT_TRUE(result->start[0] == 'a', "Character should be lowercase");
}
...
static void test_single_char_lowercase(void) {
    ...
    ASSERT_TRUE(result != NULL, "Result should not be NULL");
    ASSERT_INT_EQ(1, result->end - result->start, "Length should be 1");
    ASSERT_TRUE(result->start[0] == 'a', "Character should remain lowercase");
}
...
static void test_all_uppercase(void) {
    ...
    ASSERT_TRUE(strncmp(result->start, "hello world", 11) == 0, "Should convert to lowercase");
}
...
static void test_all_lowercase(void) {
    ...
    ASSERT_TRUE(result != NULL, "Result should not be NULL");
    ASSERT_INT_EQ(11, result->end - result->start, "Length should be 11");
    ASSERT_TRUE(strncmp(result->start, "hello world", 11) == 0, "Should remain lowercase");
}
...
static void test_long_string(void) {
    ...
    ASSERT_TRUE(strncmp(result->start, expected, 62) == 0, "Should convert all uppercase letters");
}

...
pair partition_right_branchless(int *begin, int *end) {
  int pivot = *begin;
  int *first = begin;
  int *last = end;
  ...
  if (!already_partitioned) {
    ...
    *last = temp;
    ...
    int offsets_l_storage[BLOCK_SIZE + CACHELINE_SIZE];
    int offsets_r_storage[BLOCK_SIZE + CACHELINE_SIZE];
    int *offsets_l = align_cacheline(offsets_l_storage);
    int *offsets_r = align_cacheline(offsets_r_storage);
    ...
    if (num_l) {
      offsets_l += start_l;
      while (num_l--) {
        int temp = *(offsets_l_base + offsets_l[num_l]);
    ...
    if (num_r) {
      offsets_r += start_r;
      while (num_r--) {
        int temp = *(offsets_r_base - offsets_r[num_r]);
        *(offsets_r_base - offsets_r[num_r]) = *first;
        *first = temp;
        ++first;
      }
      last = first;
